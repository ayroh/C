...
int find_if_empty(char puzzle[20][20],int length,int direction,char place[]);
int place_word(char puzzle[20][20],int length,int direction,char word[],char start[]);
int control_answer(char puzzle[20][20],char placeans[],char wordans[],int length,int *count);
...
int main() {
	...
	char puzzle[20][20],point,str[15],words[10][15],place[3],placeans[3],wordans[15];
	int j,i,b=123,character,a,printcount = 0,k = 0,cot = 0,start,direction,length,answercount = 0,indicator,wordcount = 0,answer,dircontrol = 0;
	srand(time(NULL));
	FILE *fp;
	...
	for(i = 0; i < 10; i++){		// taking 10 words from words.txt to words[10][15]
		AndAnotherOne:
		a = 1 + rand()%100;
		for(j = 1; j < a; j++) {
			fgets(str,15,fp);
		...
		while(wordcount != 10){
			...
			find_place(place);   			// place[0] contains row - place[1] contains column and if column has 2 digits place[2] contains second digit
				AnotherOne:
			indicator = find_if_empty(puzzle,length,direction,place);
				if(indicator == 0){
					...
					dircontrol = 0;
					wordcount++;
						place_word(puzzle,length,direction,words[i],place);
	...
	answer = control_answer(puzzle,placeans,wordans,length,&answercount);
	if(answer != 0){
		for(i = 0;i < 10;i++){
			if(strstr(words[i],wordans) != NULL){
				...
				place_word(puzzle,length+1,answer,words[i],placeans);
				break;
			}
		}
	}
